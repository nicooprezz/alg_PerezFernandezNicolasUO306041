...
// Clase de medición de rendimiento del algoritmo de almacenaje con Recursión y Poda (p7).
    ...
    public static void main(String[] args) {
        ...
        for (int n = 0; n <= 9; n++) {
            ...
            try (Scanner sc = new Scanner(new FileReader(ruta))) {
                ...
                int[] objetos = new int[parts.length];
                // Convierte los tokens a enteros
                for (int i = 0; i < parts.length; i++)
                    objetos[i] = Integer.parseInt(parts[i]);
                // Número total de objetos del caso de prueba
                int nObj = objetos.length;

                // Ejecuta el algoritmo el número de repeticiones indicado, midiendo cada vez
                for (int i = 1; i <= repeticiones; i++) {
                    // Reinicia los campos estáticos de AlmacenajeContenedoresRyP para cada repetición
                    // de forma que cada ejecución parta exactamente del mismo estado inicial
                    AlmacenajeContenedoresRyP.capacidadC = capacidad;
                    // Clona el array para que el algoritmo no modifique el original entre repeticiones
                    AlmacenajeContenedoresRyP.conjuntoS = objetos.clone();
                    // Cota superior inicial: en el peor caso un contenedor por objeto
                    AlmacenajeContenedoresRyP.mejorK = nObj;
                    // Reinicia la ocupación de cada contenedor a cero
                    AlmacenajeContenedoresRyP.contenedoresActuales = new int[nObj];
                    AlmacenajeContenedoresRyP.asignacion = new int[nObj];
                    AlmacenajeContenedoresRyP.mejorAsignacion = new int[nObj];
                    // Solución inicial trivial: objeto j asignado al contenedor j
                    for (int j = 0; j < nObj; j++)
                        AlmacenajeContenedoresRyP.mejorAsignacion[j] = j;

                    // Marca el instante de inicio
                    t1 = System.currentTimeMillis();

                    // Redirige la salida estándar al sumidero para que los prints no alteren el tiempo
                    PrintStream original = System.out;
                    System.setOut(silencio);
                    // Lanza el backtracking con ningún objeto asignado y ningún contenedor abierto
                    AlmacenajeContenedoresRyP.resolver(0, 0);
